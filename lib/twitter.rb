...
def tweet_hello_basic(client)
  puts 'client.update("Hello Twitter from Ruby #bonjour_monde")'
end
...
def tweet_journalists(client)
  ...
  journalists.suffle[0..4].each do |handle|
    puts 'client.update("Hello, thank you for your work, and #bonjour_monde from @The_Hacking_Pro")'
  end
end
...
def test(client)
  client.search("from:Kathatouille", since: "2018-01-01").take(500).each_with_index do |tweet, i|
    display_tweet(tweet)
    if tweet.user.name != "Mathieu Voland"
      client.favorite(tweet.user)
      puts ">> LIKED"
    end
  end
end
